...
from parsing.parse_hub import ensure_no_duplicate_hub, parse_hub
...
def parse_file() -> Map:
    ...
    for line_nb, raw in enumerate(file_content, start=1):
        ...
        if first_kw == 1:
            if keyword != "nb_drones":
                raise ParsingError(
                    line_nb,
                    "First line must define number "
                    "of drones using 'nb_drones'",
                )
        first_kw = 0
        ...
        match keyword:
            ...
            case "connection":
                ...
                if (
                    hub1_name not in current_hub_names
                    or hub2_name not in current_hub_names
                ):
                    missing = (
                        hub1_name
                        if hub1_name not in current_hub_names
                        else hub2_name
                    )
                    raise ConnectionParsingError(
                        line_nb, f"Unknown hub: '{missing}'"
                    )
                ensure_no_duplicate_connection(
                    connections, connection, line_nb
                )
            ...
            case "start_hub":
                ...
                if start_hub is not None:
                    raise ParsingError(
                        line_nb, "You must provide only one start"
                    )
                start_hub = hub
                hubs.append(hub)
            case "end_hub":
                ...
                if end_hub is not None:
                    raise ParsingError(
                        line_nb, "You must provide only one end"
                    )
                end_hub = hub
                hubs.append(hub)
            case _:
    ...
    # Resolve raw reference text tokens back down into actual Node objects
    ...
    for hub1_name, hub2_name, max_capacity in connections:
        ...
        connection_obj = Connection(
            node1=node1, node2=node2, metadata=metadata
        )
