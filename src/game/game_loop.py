...
def _resolve_hub_color(hub: object) -> pg.Color:
    """Parse hub.color into a pg.Color, falling back to red on failure."""
    try:
        raw = getattr(hub.color, "value", hub.color)  # type: ignore[attr-defined]
        return pg.Color(str(raw))
    except Exception:
        return pg.Color("red")
...
def _draw_grid(screen: pg.Surface) -> None:
    """Draw a faint line grid on the background."""
...
def _update_hub_sprite_sizes(
    ...
    """Regenerate hub surfaces and pre-compute aura surfaces."""
    size = _scale_value(BASE_HUB_DIAMETER, zoom)
    for sprite in hub_by_name.values():
        hub = sprite.hub
        has_drones = len(hub.drones or []) > 0  # type: ignore[attr-defined]
        c = _resolve_hub_color(hub)

        # --- Aura (drawn in a separate pass before hub_sprites.draw) ---
        aura_r = max(1, size // 2 + _scale_value(18, zoom))
        aura_surf = pg.Surface((aura_r * 2, aura_r * 2), pg.SRCALPHA)
        if has_drones:
            pg.draw.circle(
                aura_surf,
                (c.r, c.g, c.b, HUB_GLOW_ALPHA),
                (aura_r, aura_r),
                aura_r,
            )
...
def _draw_auras(
    ...
    """Blit pre-computed aura surfaces — must run BEFORE hub_sprites.draw."""
    for sprite in hub_by_name.values():
        aura: pg.Surface | None = getattr(sprite, "_aura", None)
        if aura is None:
            continue
        ax = sprite.rect.centerx - aura.get_width() // 2
        ay = sprite.rect.centery - aura.get_height() // 2
...
def _draw_label(
    ...
    bg_color: tuple[int, int, int, int] = LABEL_BG_COLOR,
) -> None:
    """Draw a pill badge: dark bg, colored border, colored text."""
...
def _draw_hub_labels(
    screen: pg.Surface,
    hub_by_name: dict[str, HubSprite],
    zoom: float,
) -> None:
    """Draw name pill above and drone-count badge below each hub.

    - Name:  white text, subtle white border  (same for all hubs)
    - Count: hub-colored text, hub-colored border
    """
    ...
    for sprite in hub_by_name.values():
        hub = sprite.hub
        c = _resolve_hub_color(hub)
        hub_color_rgb: tuple[int, int, int] = (c.r, c.g, c.b)
        hub_border: tuple[int, int, int, int] = (c.r, c.g, c.b, 160)
        ...
        # Name label — white text, dim white border
        _draw_label(
            screen, hub.name, name_font, name_center,  # type: ignore[attr-defined]
            text_color=LABEL_TEXT_COLOR,
            border_color=LABEL_BORDER_COLOR,
        )

        # Count badge — hub-colored text and border
        drone_count = len(hub.drones or [])  # type: ignore[attr-defined]
        _draw_label(
            screen, str(drone_count), count_font, count_center,
            text_color=hub_color_rgb,
...
def _draw_connection_drone_counts(
    ...
    """Draw amber drone-count badges at the midpoint of active connections."""
    connections: list[object] = getattr(map_, "connections", None) or []
    if not connections:
        return
    ...
    for conn in connections:
        ...
        drones: list[object] = getattr(conn, "drone", None) or []
        if not drones:
            continue  # only show badge when drones are in transit

        mid = ((s1[0] + s2[0]) // 2, (s1[1] + s2[1]) // 2)
        _draw_label(
            screen, str(len(drones)), count_font, mid,
            text_color=CONN_BADGE_TEXT,
...
def game_loop(map_: Map) -> None:
    ...
    base_positions: dict[str, tuple[float, float]] = _compute_base_hub_pixels(map_, screen)
